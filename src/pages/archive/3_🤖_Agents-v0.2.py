            ...
            with st.expander(f"{agent.name} (v{agent.version})"):
                ...
                if agent.code_snippets:
                    ...
                    for i, snippet in enumerate(agent.code_snippets):
                        ...
                        st.markdown("---")  # Add a separator between snippets
...
with tab2:
    ...
    # Add LLM button outside form
    if st.button("Add LLM", key="add_llm_button"):
        st.session_state.llm_count += 1
        st.rerun()
    
    # Code snippet counter outside form
    ...
    # Add Code Snippet button outside form
    if st.button("Add Code Snippet", key="add_snippet_button"):
        st.session_state.code_snippet_count += 1
        st.rerun()
    ...
    with st.form("agent_form"):
        ...
        for i in range(st.session_state.llm_count):
            with st.container():
                ...
                notes = st.text_area("Notes (optional)", value=default_notes, key=f"llm_notes_{i}", height=70)
